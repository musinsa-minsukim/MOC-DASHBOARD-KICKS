...
def get_store_moves() -> pd.DataFrame:
    """매장 STO 이동중(입고예정/출고예정). 캐시 없으면 빈 df."""
    try:
        return _read_cached("store_moves")
    except OSError:
        return pd.DataFrame(columns=["store_name", "goods_no", "option", "in_qty", "out_qty", "hist_recv", "first_recv_date", "last_out_date"])

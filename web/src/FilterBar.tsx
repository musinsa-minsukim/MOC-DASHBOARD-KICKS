...
export default function FilterBar({
  ...
  const active =
    ...
    f.cat_large.length || f.cat_medium.length || f.concept.length || f.md.length || uidCount || nameOn || f.running;
  const activeN =
    f.biz.length + f.type.length + f.store.length + f.brand.length + f.brand_ex.length + f.cat_top.length +
    f.cat_large.length + f.cat_medium.length + f.concept.length + f.md.length + (uidCount ? 1 : 0) + (nameOn ? 1 : 0) + (f.running ? 1 : 0);
  ...
  return (
    <Card>
      ...
      <CardBody className={`${open ? "flex" : "hidden"} flex-wrap items-end gap-x-4 gap-y-3 p-4 lg:flex`}>
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-1.5">
            ...
            {meta.concepts && meta.concepts.length > 0 && (
              <MultiSelect label="컨셉" options={meta.concepts} value={f.concept} onChange={(v) => set({ concept: v })} />
            )}
        ...
        {active ? (
          <button
            onClick={() => set({ biz: [], type: [], store: [], brand: [], brand_ex: [], cat_top: [], cat_large: [], cat_medium: [], concept: [], md: [], goods: "", name_like: "", running: false })}

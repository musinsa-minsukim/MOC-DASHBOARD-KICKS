...
export default function Inventory({ meta, dark, filters, onPick }: { meta: Meta; dark: boolean; filters: Filters; onPick?: (k: keyof Filters, v: string) => void }) {
  ...
  const invCols = useMemo(() => [
    ...
    colText("입고구분", "입고구분", {
      minWidth: 84, headerTooltip: "이동중 입고 기준 · 신규입고=그 매장에 입고 이력이 없는 첫 입고 / 필업=입고 이력 있음(완판 후 재입고 포함) / (공란)=입고 없음. ※현재 재고 0이 아니라 '입고 이력'으로 판정",
  ...
  const brandTotal = useMemo(() => {
    ...
    return [{ name: "합계", total: s("total"), share: ss, gmv: s("gmv"), gmv_share: gs,
              ...
              in_sku: s("in_sku"), fillup_sku: s("fillup_sku"), out_sku: s("out_sku"), ttl_sku: s("ttl_sku") }];
  }, [brandRows]);
  const brandTblCols = useMemo(() => [
    ...
    colNum("in_sku", "입고예정", "int", { minWidth: 82, headerTooltip: "STO 창고→매장 이동중 중 신규(현재 매장 미보유) 컬러-SKU만 · 필업 제외" }),
    colNum("fillup_sku", "필업", "int", { minWidth: 68, headerTooltip: "입고예정 중 이미 매장에 있는 SKU 보충(신규 아님, TTL 미반영)" }),
    colNum("out_sku", "출고예정", "int", { minWidth: 82, headerTooltip: "STO 매장→창고 반품 이동중(출고 예정) 컬러-SKU" }),
    colNum("ttl_sku", "TTL SKU", "int", { minWidth: 88, headerTooltip: "마감정상(컬러SKU) + 신규입고예정 − 출고예정 − 브로큰" }),
  ...
  const invTotal = useMemo(() => {
    ...
    let jaego = 0, expected = 0, outgoing = 0; for (const c of hubCols) t[c] = 0;
    const seen = new Set<string>();
    for (const r of rr) {
      jaego += Number(r["점재고"]) || 0;          // 점재고·입고예정·출고예정은 매장×옵션 distinct → 전행 합산
      expected += Number(r["입고예정"]) || 0;
      outgoing += Number(r["출고예정"]) || 0;
      const bc = String(r.__bc ?? "");
      if (bc && seen.has(bc)) continue;   // 허브는 barcode당 1회
      if (bc) seen.add(bc);
      for (const c of hubCols) t[c] += Number(r[c]) || 0;
    }
    t["점재고"] = jaego; t["입고예정"] = expected; t["출고예정"] = outgoing;
    return [t];
  }, [d, hubcols]);
      ...
      {!d ? (
        ...
        <>
          ...
          <div className="rounded-lg border border-slate-200 bg-slate-50 px-4 py-2.5 text-xs leading-relaxed text-slate-500 dark:border-slate-700 dark:bg-slate-800/40 dark:text-slate-400">
            ...
            <br />
            <span className="font-semibold text-amber-600 dark:text-amber-400">입고예정·출고예정·입고구분(상품옵션별)</span> · <b className="text-emerald-600 dark:text-emerald-400">입고예정</b>=그 매장으로 오는 이동중(STO) 수량 / <b className="text-rose-600 dark:text-rose-400">출고예정</b>=창고로 나가는 반품. <b className="text-emerald-600 dark:text-emerald-400">신규입고</b>=그 매장에 <b>입고 이력이 없는 첫 입고</b> / <b className="text-amber-600 dark:text-amber-400">필업</b>=<b>입고 이력 있음</b>(완판됐다가 재입고도 필업) / <b>(공란)</b>=입고 없음. (현재 재고 0이 아니라 입고확정 이력 기준) <b>출고예정</b>=반품 <b>위탁만</b>(출고요청 단계부터, requested−received). <b>매입은 제외</b> — 창고 입고확정을 매장별로 차감할 신호가 없어 누적·과대가 되고 조회도 무거워 반영하지 않음.
            <br />
